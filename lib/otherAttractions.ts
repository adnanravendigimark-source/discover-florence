...
// Other Attractions — replaces the old inline, per-museum
// `other_attractions_json` list (a simple 5-field RepeatableList embedded
// directly in MuseumForm.tsx). Managed exactly like a museum's own
// tours/tickets (museum_tours): each record belongs to exactly one museum,
// added and edited from that museum's own "Manage Other Attractions"
// screen (see /admin/attractions/[museumId] and MuseumAttractionForm.tsx).
// Nothing is looked up or guessed — name, photo, price, and the booking
// link are all typed in by hand, same as a tour. The old
// other_attractions_json column on `museums` is left untouched (never
// dropped, per this repo's migration policy) — it's simply no longer read
// or written by the public page or the admin form.
